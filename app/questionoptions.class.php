...
class questionoptions {
    ...
    public function option_id() {
        return $this->summary("option_id", 0);
    }
    public function is_valid() {
        return !empty($this->summary);
    }

    public static function create($qid, $title, $value, $option_id){
        if (!empty($qid)) {
            file_put_contents("./log_" . date("Y-m-d") . ".txt",  "\n".date("H:i:s", time()).':'.__METHOD__.':'."qid:$qid, title:$title value:$value\r\n", FILE_APPEND);
            $id = db_questionoptions::inst()->add_options($qid, $title, $value, $option_id);
            $questionoptions = db_questionoptions::inst()->get_options_by_id($id);
        }
        //logging::d("createPJT", "muffininfos: $muffininfos");
        return new questionoptions($questionoptions);
